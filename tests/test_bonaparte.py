...
class BonaparteRunnerTests(unittest.TestCase):
    ...
    def test_review_receipts_remain_bounded_and_schema_validated(self):
        accepted = {"x": "a" * 4087}
        serialized = RUNNER.serialize_review_receipt(accepted)
        self.assertEqual(
            len((serialized + "\n").encode("utf-8")), RUNNER.RECEIPT_MAX_BYTES
        )
        with self.assertRaisesRegex(RuntimeError, "receipt exceeded 4 KiB"):
            RUNNER.serialize_review_receipt({"x": "a" * 4088})
        with self.assertRaisesRegex(RuntimeError, "invalid receipt"):
            RUNNER.validate_schema(
                {**delivery_receipt("review"), "state": "invalid"},
                RUNNER.RECEIPT_SCHEMA,
            )

    def test_review_coordinator_stderr_remains_private(self):
        result = subprocess.CompletedProcess([], 23, stderr="private detail")
        with (
            mock.patch.object(RUNNER, "find_codex", return_value="/bin/codex"),
            ...
            RUNNER.run_phase(ROOT, "review", "Continue.", SESSION_ID)
